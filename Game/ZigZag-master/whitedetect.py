...
def CheckWhite(img, x, y, far, radius, count, show=False):
    ...
    if show:
        ...
        img = cv2.circle(img, (x+far, y), radius=2,
                         color=(0, 0, 255), thickness=-1)
        img = cv2.circle(img, (trackX, trackY), radius=2,
                         color=(0, 255, 0), thickness=-1)
        img = cv2.circle(img, (trackX2, trackY2), radius=2,
                         color=(0, 255, 0), thickness=-1)
...
if __name__ == "__main__":
    img = cv2.imread(
        "C:\Code\OpenCV\Game\ZigZag-master\LineTests\151color_233-254_23_HitLeft.png", cv2.IMREAD_COLOR)
