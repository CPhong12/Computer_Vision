...
from whitedetect import CheckWhite
...
while True:
    ...
    mask = cv2.inRange(hsv, np.array(
        [153, 96, 175]), np.array([156, 255, 255]))
    ...
    HoughLines = cv2.HoughLinesP(
        lines, 1, np.pi/180, threshold=19, minLineLength=20, maxLineGap=1)
    ...
    circles = cv2.HoughCircles(
        gray, cv2.HOUGH_GRADIENT, 1, 20, param1=50, param2=18, minRadius=10, maxRadius=14)
    if circles is not None:
        ...
        for pt in circles[0, :]:
            ...
            if answer:
                ...
                time.sleep(0.02)
                image_holder.append(
                    (img, f"{count}--CHECKWHITE-CLICK{goingRight}-{x}-{y}-{pushFar}-{r}.png"))
                continue
            ...
            if sum(black[y+lookUp, x+r+pushShort:x+pushFar]) > 0 and goingRight:
                ...
                goingRight = False
                image_holder.append(
                    (img, f"{count}color{x}-{y}-{r}_{x+r+pushShort}-{x+pushFar}_{y+lookUp}_HitRight.png"))
            ...
            # if we find edge to left click with lookahead
                ...
                goingRight = True
                image_holder.append(
                    (img, f"{count}color{x}-{y}-{r}_{x-pushFar}-{x-r-pushShort}_{y+lookUp}_HitLeft.png"))
            ...
            # if we find edge to right click
                ...
                goingRight = False
                image_holder.append(
                    (img, f"{count}color{x}-{y}-{r}_{x+r+pushShort}-{x+pushFar}_{y+lookUp}_HitRightY2.png"))
            ...
            # if we find edge to left click
                ...
                goingRight = True
                image_holder.append(
                    (img, f"{count}color{x}-{y}-{r}_{x-pushFar}-{x-r-pushShort}_{y+lookUp}_HitLeftY2.png"))
            else:
                image_holder.append(
                    (img, f"{count}color_nohit-{x}-{y}-{r}.png"))
    ...
    # Use the balls last position to make an educated guess.
        ...
        if goingRight:
            ...
            answer = CheckWhite(color, x, y, pushFar, r, count)
            image_holder.append(
                (img, f"{count}color_{x}_{y}_{pushFar}_{answer}GUESS.png"))
        else:
            x = x - 1
            answer = CheckWhite(color, x, y, -pushFar, r, count)
            image_holder.append(
