...
def take_input():
    ...
    file = open("D:\Code\OpenCV\Project\input.txt", "r")
...
def find_intersection_plane(ray, plane):
    ...
    t = np.dot(convert_unit_vector(plane.normal), (np.subtract(
        plane.point, ray.point))) / np.dot(ray.dir, plane.normal)
...
def cast_ray(ray, depth):
    ...
    if intersection[1] is not None and depth < max_depth:
        ...
        ambient_col = np.multiply(
            ambient_color, ambient_component, np.divide(intersection[1].color, 256))
        unit_normal = convert_unit_vector(
            intersection[1].getNormal(intersection_point))
        diffuse_col = np.multiply(intersection[1].k_diffuse * max(0, np.dot(unit_light_dir, unit_normal)),
                                  intersection[1].color)

        new_ray = reflect_ray(ray, intersection[1].getNormal(
            intersection_point), intersection_point)
        color = ambient_col + diffuse_col * light
        color += np.multiply(intersection[1].k_reflect,
                             cast_ray(new_ray, depth + 1)) / (depth + 1)
